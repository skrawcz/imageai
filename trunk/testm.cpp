...
**  file "easy.xml".
** UPDATE: By: Ross Teske <rteske@stanford.edu>
**         Added the -m option to output an mpeg movie.
**         This was based on the following reference page:
**         http://www.hci.iastate.edu/575x/doku.php?id=wenchiehchang:homework:hw1
*****************************************************************************/
...
void usage() {
    ...
    cerr << "    -m <filename>  :: output a mpeg movie of the results" << endl;
    cerr << endl;
}
...
int main(int argc, char *argv[])
{
    ...
    outputMovieFile = NULL;     // set using -m <filename>
    bVerbose = false;           // turn on with -v
    bShowVideo = true;          // turn off with -x
    bMovie = false;             // turn on with -m
    ...
    while (argc-- > 2) {
        if (!strcmp(*args, "-c")) {
            ...
            bMovie = true;
            argc--; args++;
            if (outputMovieFile != NULL) {
                usage();
                return -1;
            }
            outputMovieFile = *args;
        } else {
    ...
    if (bVerbose) {
        ...
        cout << "Movie Output file:  "
             << (outputMovieFile == NULL ? "(none)" : outputMovieFile) << endl;
    ...
    while ((frame = cvQueryFrame(capture)) != NULL) {
        ...
        if(bMovie)
        {
            sprintf(outputPath, "tempoutputframes/frame%03d.png", frameCount);
            cvSaveImage(outputPath, frameCopy);
        }
    ...
    if(bMovie)
    {
        char * command1 = "/afs/ir/class/cs221/vision/bin/ffmpeg -i tempoutputframes/frame%03d.png ";
        char * command = new char[strlen(command1) + strlen(outputMovieFile)];
        strcpy(command, command1);
        strcat(command, outputMovieFile);
        system(command);
        delete(command);
        system("rm -f -r tempoutputframes");
    }
